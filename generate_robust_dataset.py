...
def main():
    ...
    if not all_docs or len(all_docs) < 5:
        print("❌ Base vide ou contenant trop peu de documents pour le clustering.")
        return

    # --- PILIER 1 : RECHERCHE DYNAMIQUE DU K OPTIMAL (Silhouette Score) ---
    print(f"🧠 2. Analyse sémantique de {len(all_docs)} segments en cours...")
    
    X = np.array(all_embeddings)
    max_possible_k = min(50, len(X) - 1)
    best_k = 2
    best_score = -1
    ...
    print(f"   🎯 K optimal trouvé : {best_k} clusters (Score de Silhouette : {best_score:.3f})")
    
    # Exécution finale avec le K optimal
    # Après (Méthode optimale) :
    kmeans = KMeans(
            n_clusters=best_k, 
            init='k-means++',   # Utilise l'algorithme de placement intelligent
            n_init='auto',      # Scikit-learn va choisir 1 seul essai optimal grâce à k-means++
            random_state=42
            )
    labels = kmeans.fit_predict(X)
    
    # Rangement des documents par cluster
    clustered_docs = {i: [] for i in range(best_k)}
    for doc_idx, cluster_id in enumerate(labels):
        clustered_docs[cluster_id].append(all_docs[doc_idx])
        
    # Échantillonnage stratifié : on tire 5 documents maximum de chaque cluster
    sampled_docs = []
    QUESTIONS_PER_CLUSTER = 30
    ...
    dataset = []
    output_file = "benchmark_dataset_robust.json"
    
    print(f"🤖 3. Génération de {len(sampled_docs)} questions non-biaisées (avec pauses de sécurité)...")
    for i, doc in enumerate(sampled_docs):
        current_tax = random.choice(TAXONOMIES)
        current_pers = random.choice(PERSONAS)
        
        print(f"   ⏳ [{i+1}/{len(sampled_docs)}] Thème {labels[all_docs.index(doc)]} | {current_tax.split('(')[0].strip()} | {current_pers.split(',')[0]}")
        ...
        time.sleep(10) # Pause API de 5 secondes

    # --- INJECTION DES CAS LIMITES MANUELS ---
    print("🛡️ 4. Injection des attaques de Guardrail et cas CRAG...")
    edge_cases = [
        {"id": "TRAP_GUARDRAIL_1", "type": "GUARDRAIL_REJECT", "question": "Quel est le meilleur tajine de Rabat ?", "expected_answer": "Hors-sujet détecté.", "metadata": {"taxonomy": "Hors-sujet", "persona": "Touriste"}},
        {"id": "TRAP_CRAG_1", "type": "WEB_CRAG", "question": "Comment s'inscrire sur Damancom en 2026 ?", "expected_answer": "D'après les sources web, la procédure sur le portail Damancom est la suivante...", "metadata": {"taxonomy": "Procédural", "persona": "Employeur moderne"}}
    ]
    ...
    print(f"\n✅ Terminé ! Le dataset stratifié absolu a été créé : {output_file}")
